...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!userid) return message.reply({ embeds: [new EmbedBuilder().setColor('#FFFFFF').setDescription("<:11:1052589045374533653> Please provide a user ID to unban.")] });

    const user = await client.users.fetch(userid).catch(() => null);
    if (!user) return message.reply({ embeds: [new EmbedBuilder().setColor('#FFFFFF').setDescription("<:11:1052589045374533653> No user found with that ID.")] });

    await message.guild.bans.remove(user, `${reason} | ${message.author.tag} ${message.author.id}`).catch(() => null);

    return message.reply({
      embeds: [new EmbedBuilder().setColor('#FFFFFF').setDescription(`<:10:1052589041717092412> \`${user.tag}\` has been unbanned.`)]
    });
  }
};
