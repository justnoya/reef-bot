...
module.exports = {
  ...
  description: 'Server-mute a member in a voice channel',
  ...
  run: async (client, message, args) => {
    ...
    if (target.voice.serverMute) return err('That member is already server-muted.');

    const reason = args.slice(1).join(' ') || 'No reason provided';
    await target.voice.setMute(true, `${reason} | By ${message.author.tag}`);

    const container = new Container()
      .setAccentColor(accent)
      .addComponents(
        new TextDisplay(`## <:moderation:1516337259157131365> Member Voice-Muted`),
